...
fn main() -> io::Result<()>{
    ...
    let mut pos_head: (i32, i32) = (500, 500); //pos x y
    let mut pos_tail: (i32, i32) = (500, 500); //pos x y
    for line in reader.lines(){
        let line=line?;
        let line: Vec<&str> = line.split(" ").collect();
        let num = line[1].parse::<i32>().unwrap();
        for _i in 0..num{
            move_head(&mut pos_head, line[0]);
            move_tail(&pos_head, &mut pos_tail);
            visited[pos_tail.0 as usize][pos_tail.1 as usize] = true;
        }
    }
