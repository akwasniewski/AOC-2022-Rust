...
fn main() -> io::Result<()>{
    ...
    let mut items_held: [Vec<i128>; 8] =  Default::default();
    let mut inspections: [i128;8] = [0;8];
    let mut cur_monkey = 0;     
    for line in reader.lines()
    {
        let line = line?;
        let line: Vec<i128> = line.split(" ").map(|x| x.parse::<i128>().unwrap()).collect();
    ...
    for _t in 0..10000{
        for cur_monkey in 0..8{
            ...
            for i in 0..monkey_len{
                ...
                let item = monkey_calc(cur_monkey as i128, item);
                inspections[cur_monkey]+=1;
                let where_thrown = monkey_where(cur_monkey as i128, item);
...
fn monkey_calc(monkey: i128, mut val: i128) ->i128{
    match monkey{
        0=>val=val*7,
        1=>val=val*13,
        2=>val=val+1,
